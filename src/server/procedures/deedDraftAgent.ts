...
// QD generate input is NOT a discriminated union: it keeps `deedType` a free `z.string()` (so the friendly
// QUICK_DEED_TYPE_NOT_WIRED guard, not zod, handles any unwired/unrecognized type) and carries the gift fields at
// the top level + ONE nested payload per wired category. It is defined AFTER every per-category input schema —
// see `quickDeedGenerateInput` ("Quick Deed multi-category generate input") below the into-trust schema.
...
/** Registry keys for the remaining Quick-Deed fast-lane categories (Tier-3 + C-series), each reusing its
 *  matter-scoped build core. These MUST match deedTypeRegistry.ts exactly. */
export const QUICK_DEED_INTO_LLC_TYPE = 'deed_into_llc';
export const QUICK_DEED_OUT_OF_LLC_TYPE = 'deed_out_of_llc';
export const QUICK_DEED_TOD_TYPE = 'deed_tod';
export const QUICK_DEED_CONFIRMATION_TYPE = 'deed_of_confirmation';
export const QUICK_DEED_INTO_TRUST_TYPE = 'deed_into_trust';
 ...
 *  + a quickDeedGenerateInput nested payload; the selector enables automatically). Today: gift, seller-side,
 *  into-LLC, out-of-LLC, TOD, confirmation, into-trust — the whole built registry. */
export const WIRED_QUICK_DEED_TYPES: ReadonlySet<string> = new Set([
  QUICK_DEED_GIFT_TYPE,
  QUICK_DEED_SELLER_TYPE,
...
// `quickDeedGenerateInput` is defined AFTER all the per-category input schemas (it nests one payload per wired
// category, and the into-LLC/out-of-LLC/TOD/confirmation/into-trust input schemas live further down) — see
// "Quick Deed multi-category generate input" below the into-trust schema.
 ...
 *  testing. */
export function buildTodDocNotes(draft: DeedTodResult, categoryNotesRendered = '', extraNotes: readonly string[] = []): string {
  return [
    'Generated by DEED-DRAFT-AGENT-1 (deterministic). The attorney reviews/edits/approves; this draft is never auto-recorded, filed, or sent.',
    ...extraNotes,
    ...draft.advisories,
...
export function buildConfirmationDocNotes(draft: DeedConfirmationResult, categoryNotesRendered = '', extraNotes: readonly string[] = []): string {
  return [
    'Generated by DEED-DRAFT-AGENT-1 (deterministic). The attorney reviews/edits/approves; this draft is never auto-recorded, filed, or sent.',
    ...extraNotes,
    ...draft.advisories,
...
export function buildIntoLlcDocNotes(draft: DeedIntoLlcResult, categoryNotesRendered = '', extraNotes: readonly string[] = []): string {
  return [
    'Generated by DEED-DRAFT-AGENT-1 (deterministic). The attorney reviews/edits/approves; this draft is never auto-recorded, filed, or sent.',
    ...extraNotes,
    ...draft.advisories,
 ...
 *  deed). Exported for testing. */
export function buildOutOfLlcDocNotes(draft: DeedOutOfLlcResult, categoryNotesRendered = '', extraNotes: readonly string[] = []): string {
  return [
    'Generated by DEED-DRAFT-AGENT-1 (deterministic). The attorney reviews/edits/approves; this draft is never auto-recorded, filed, or sent.',
    ...extraNotes,
    ...draft.advisories,
...
// ── Quick Deed multi-category generate input ──────────────────────────────────────────────────────────────────
//
// The fast-lane generate input: the gift fields at the TOP LEVEL (the original v1 shape, unchanged) + a free-string
// deedType (so the dispatch guard, not zod, produces the friendly unwired/unrecognized message) + ONE nested payload
// per wired category. Seller-side reuses the top-level gift party/grantee-address fields (its party shape is
// gift-compatible), so its nested payload OMITS them and defaults to {}. The remaining categories carry DISTINCT
// party/structure shapes that do NOT map onto the gift fields, so each nests its WHOLE matter-scoped input minus
// matterId and is `.optional()` (NOT `.default({})`): those schemas have REQUIRED fields — notaryJurisdiction /
// returnTo / transferor / archetype / exemplar / instrumentDate — so a `.default({})` would FAIL validation on every
// NON-matching generate (e.g. a plain gift generate). Absent => undefined; the matching dispatch branch supplies the
// matterId and reuses that category's existing build core. Add a key to WIRED_QUICK_DEED_TYPES, a nested payload
// here, AND a dispatch branch in quickDeed.generate in lockstep (the three move together or the selector/guard drift).
const quickDeedIntoLlcInput = createIntoLlcDraftInput.omit({ matterId: true });
const quickDeedOutOfLlcInput = createOutOfLlcDraftInput.omit({ matterId: true });
const quickDeedTodInput = createTodDraftInput.omit({ matterId: true });
const quickDeedConfirmationInput = createConfirmationDraftInput.omit({ matterId: true });
const quickDeedIntoTrustInput = createIntoTrustDraftInput.omit({ matterId: true });
...
const quickDeedGenerateInput = createGiftDraftInput.extend({
  ...
  intoLlc: quickDeedIntoLlcInput.optional(),
  outOfLlc: quickDeedOutOfLlcInput.optional(),
 ...
 *  Exported for testing. */
export function buildIntoTrustDocNotes(draft: DeedIntoTrustResult, categoryNotesRendered = '', extraNotes: readonly string[] = []): string {
  return [
    'Generated by DEED-DRAFT-AGENT-1 (deterministic). The attorney reviews/edits/approves; this draft is never auto-recorded, filed, or sent.',
    ...extraNotes,
    ...draft.advisories,
...
// ── Quick Deed multi-category dispatch helpers (the {status:'OK'|'WITHHELD', deed?} categories) ────────────────
//
// The into-LLC / out-of-LLC / TOD / confirmation / into-trust matter procedures share the {status:'OK'|'WITHHELD',
// deed?} result shape (NOT seller-side's failedClosed/text). Their Quick-Deed dispatch branches reuse the category's
// build core, then map the result into the SAME unified quickDeed.generate return shape the gift + seller-side
// branches use (so the tRPC return type stays a single object union and the client reads one shape). A WITHHELD
// result NEVER persists a void deed (fail-closed); the assembler's flags surface as `failures` and its advisories
// ride `notes`.

/** The unified WITHHELD result for the {status:'OK'|'WITHHELD'} Quick-Deed categories — fail-closed, no document
 *  persisted. `flags` are the assembler's fail-closed reasons (surfaced as `failures`); `advisories` ride `notes`. */
function quickDeedWithheldResult(
  matterId: string,
  flags: readonly string[],
  advisories: readonly string[],
  warnings: readonly string[],
  conflictsBypassed: boolean,
) {
  return {
    documentId: null as string | null,
    matterId,
...
/** Persist an OK {status:'OK'} Quick-Deed category draft (the SAME insertDocument -> version -> current-pointer
 *  chain the matter procedures + the seller-side branch use) and return the unified quickDeed.generate OK shape.
 *  `docNotes` already carries the conflicts-bypass stamp (threaded via build<Cat>DocNotes extraNotes). */
async function persistQuickDeedCategoryDeed(
  userId: string,
  matterId: string,
  title: string,
  content: string,
  ...
  const versionNumber = await getNextVersionNumber(doc.id, userId);
  const version = await insertVersion({
    userId,
    documentId: doc.id,
    versionNumber,
  ...
  await updateDocumentCurrentVersion(doc.id, userId, version.id);
  return {
    documentId: doc.id as string | null,
    matterId,
    versionId: version.id as string | null,
...
export const quickDeedRouter = router({
  ...
  listDeedTypes: protectedProcedure.query(() => {
    ...
    // The Quick Deed dispatch now wires the whole built registry; `quickDeedGenerates` reads the SAME
    // WIRED_QUICK_DEED_TYPES set the generate guard uses (no drift) — any still-unwired key lists but does not generate.
   ...
   * assertDeedDraftingAllowed({ bypassConflicts: true })). Dispatches every built category (gift, seller-side,
   * into-LLC, out-of-LLC, TOD, confirmation, into-trust); any still-unwired registry key is rejected with
   * QUICK_DEED_TYPE_NOT_WIRED. The gift branch reuses the EXACT gift core
  ...
  generate: protectedProcedure.input(quickDeedGenerateInput).mutation(async ({ ctx, input }) => {
    ...
    // ── Into-LLC dispatch (C3; QUITCLAIM, § 58.1-811(A)(10)) — reuses the matter-scoped into-LLC core. The
    //    assembler returns {status:'OK'|'WITHHELD', deed?} (NOT seller-side's failedClosed/text), so we branch on
    //    status==='WITHHELD' and fail closed (never persist a void deed). The nested payload carries the WHOLE
    //    category input minus matterId (its grantor/notary shapes do not map onto the gift top-level fields). ──
    if (input.deedType === QUICK_DEED_INTO_LLC_TYPE) {
      if (!input.intoLlc) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'QUICK_DEED_MISSING_PAYLOAD: deed_into_llc requires the intoLlc fields.' });
      }
      const intoLlcInput: CreateIntoLlcDraftInput = { ...input.intoLlc, matterId: input.matterId };
      const { facts, draft } = buildIntoLlcDraft(materialRows, intoLlcInput);
      if (draft.status === 'WITHHELD' || !draft.deed) {
        return quickDeedWithheldResult(input.matterId, draft.flags, draft.advisories, facts.warnings, conflictsBypassed);
      }
      const title = (input.title ?? '').trim() || 'Deed Into an LLC';
      const docNotes = buildIntoLlcDocNotes(draft, '', conflictsNotes);
      return persistQuickDeedCategoryDeed(ctx.userId, input.matterId, title, draft.deed.fullText, docNotes, draft.advisories, facts.warnings, conflictsBypassed);
    }

    // ── Out-of-LLC dispatch (C4; Special Warranty, § 58.1-811(A)(11), LLC-by-members signature block). ──
    if (input.deedType === QUICK_DEED_OUT_OF_LLC_TYPE) {
      if (!input.outOfLlc) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'QUICK_DEED_MISSING_PAYLOAD: deed_out_of_llc requires the outOfLlc fields.' });
      }
      const outOfLlcInput: CreateOutOfLlcDraftInput = { ...input.outOfLlc, matterId: input.matterId };
      const { facts, draft } = buildOutOfLlcDraft(materialRows, outOfLlcInput);
      if (draft.status === 'WITHHELD' || !draft.deed) {
        return quickDeedWithheldResult(input.matterId, draft.flags, draft.advisories, facts.warnings, conflictsBypassed);
      }
      const title = (input.title ?? '').trim() || 'Deed Out of an LLC';
      const docNotes = buildOutOfLlcDocNotes(draft, '', conflictsNotes);
      return persistQuickDeedCategoryDeed(ctx.userId, input.matterId, title, draft.deed.fullText, docNotes, draft.advisories, facts.warnings, conflictsBypassed);
    }

    // ── TOD dispatch (C5; Revocable Transfer on Death Deed, § 58.1-811(J), death-effective, no warranty). ──
    if (input.deedType === QUICK_DEED_TOD_TYPE) {
      if (!input.tod) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'QUICK_DEED_MISSING_PAYLOAD: deed_tod requires the tod fields.' });
      }
      const todInput: CreateTodDraftInput = { ...input.tod, matterId: input.matterId };
      const { facts, draft } = buildTodDraft(materialRows, todInput);
      if (draft.status === 'WITHHELD' || !draft.deed) {
        return quickDeedWithheldResult(input.matterId, draft.flags, draft.advisories, facts.warnings, conflictsBypassed);
      }
      const title = (input.title ?? '').trim() || 'Revocable Transfer on Death Deed';
      const docNotes = buildTodDocNotes(draft, '', conflictsNotes);
      return persistQuickDeedCategoryDeed(ctx.userId, input.matterId, title, draft.deed.fullText, docNotes, draft.advisories, facts.warnings, conflictsBypassed);
    }

    // ── Confirmation dispatch (C1; confirms title already vested by operation of law — attorney-load-bearing
    //    chain-of-title recitals; carry the S5 survivorship-derivation fail-closed caveat unchanged). ──
    if (input.deedType === QUICK_DEED_CONFIRMATION_TYPE) {
      if (!input.confirmation) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'QUICK_DEED_MISSING_PAYLOAD: deed_of_confirmation requires the confirmation fields.' });
      }
      const confirmationInput: CreateConfirmationDraftInput = { ...input.confirmation, matterId: input.matterId };
      const { facts, draft } = buildConfirmationDraft(materialRows, confirmationInput);
      if (draft.status === 'WITHHELD' || !draft.deed) {
        return quickDeedWithheldResult(input.matterId, draft.flags, draft.advisories, facts.warnings, conflictsBypassed);
      }
      const title = (input.title ?? '').trim() || 'Deed of Confirmation';
      const docNotes = buildConfirmationDocNotes(draft, '', conflictsNotes);
      return persistQuickDeedCategoryDeed(ctx.userId, input.matterId, title, draft.deed.fullText, docNotes, draft.advisories, facts.warnings, conflictsBypassed);
    }

    // ── Into-Trust dispatch (C2; INTO a revocable living trust). The load-bearing `trusteesRecital` is
    //    ATTORNEY-SUPPLIED VERBATIM (the build core never auto-fabricates it from the extracted trust facts); the
    //    B6 caution (no recordable-floor gate on trust) rides the build core unchanged. ──
    if (input.deedType === QUICK_DEED_INTO_TRUST_TYPE) {
      if (!input.intoTrust) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'QUICK_DEED_MISSING_PAYLOAD: deed_into_trust requires the intoTrust fields.' });
      }
      const intoTrustInput: CreateIntoTrustDraftInput = { ...input.intoTrust, matterId: input.matterId };
      const { facts, draft } = buildIntoTrustDraft(materialRows, intoTrustInput);
      if (draft.status === 'WITHHELD' || !draft.deed) {
        return quickDeedWithheldResult(input.matterId, draft.flags, draft.advisories, facts.warnings, conflictsBypassed);
      }
      const title = (input.title ?? '').trim() || 'Deed Into Trust';
      const docNotes = buildIntoTrustDocNotes(draft, '', conflictsNotes);
      return persistQuickDeedCategoryDeed(ctx.userId, input.matterId, title, draft.deed.fullText, docNotes, draft.advisories, facts.warnings, conflictsBypassed);
    }
