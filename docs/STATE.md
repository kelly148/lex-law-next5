...
## 2026-06-09 (MATERIALS-DROPZONE-1 Increment B COMPLETE + MERGED — in-process image + scanned-PDF OCR; NOT deployed; live OCR UAT owed)

**Disposition.** Increment B of the materials drop-zone UAT finding: OCR for **PNG/JPEG and scanned (no-text-layer) PDFs**. Operator approved the gated dependency decision (engine + rasterizer + bundled model) then **`operator approve accept:`**. **Squash-merged PR #242 → `main` `de770a4`** (CI green on `main`: Lint + Type Check + Tests); branch `lex-next/materials-dropzone-1-ocr` deleted. **NOT deployed** (Railway auto-deploy OFF; merge ≠ deploy). Built off the post-Increment-A `main` (rebased onto `6ed11f8`, the #241 DOC-CLIENT-TARGET-1 test).

**What shipped.** Fully **in-process** OCR — pure JS+WASM, **no native addon, no system package, NO client-document egress, NO runtime CDN**. **tesseract.js@7** (Apache-2.0) with `langPath` pinned to a **bundled local model** (`assets/tessdata/eng.traineddata`, ~4 MB 'fast' eng) + `cacheMethod: 'none'` (tesseract fetches from a CDN by default — overridden); WASM core resolved by tesseract.js's own Node `require`. **@hyzyla/pdfium@2.1** (MIT, PDFium-in-WASM) rasterizes scanned-PDF pages → **pngjs@7** (MIT) PNG → OCR (mupdf rejected: AGPL). Dockerfile runner COPYs `assets/` + **build-time asserts the model exists**. Upload handler: an image, or a PDF whose unpdf text layer is empty (`partial`), is stored `extractionStatus='processing'` and the request returns immediately; OCR runs **fire-and-forget in-process** on the upload buffer (no blob storage), updating the row to `extracted`/`low_confidence`/`failed`. New enum values `processing` + `low_confidence` (server `schema.ts` + shared `matters.ts` in lockstep + additive **migration 0024** on the pre-deploy allowlist). Single shared tesseract worker + serial queue (bounded memory) + in-flight cap (8). Client: `OCR…`/`low confidence` chips + poll-while-processing.

**Honesty floor (hard).** A sub-floor (<60%) or empty OCR result is classified `low_confidence` with **`textContent` NULLED at the data layer** — garbled OCR is never persisted, so it cannot reach drafting, regeneration, review, outline, *or* the assessment (all gate inclusion on non-empty text). Pinned by a status-gate test. A **19-agent adversarial review** (5 lenses + per-finding verification) ran pre-commit and caught two serious issues now fixed: the OCR model was untracked in git (would have shipped a dead/CDN OCR), and low_confidence text was reaching the **drafting/review** context assembler (a separate path from the assessment) — closed by nulling the text. Also fixed: an inverted-color rasterizer (pdfium delivers RGBA, not BGRA), a poisoned worker-init promise, an unbounded in-flight queue, worker teardown on shutdown; added the migration source-audit + strengthened honesty tests.

**Build state.** `main` = `de770a4`; prod = `abd8434` (Increment A is live; Increment B + #241 are merged-not-deployed). **Open items / gate residuals:** (1) **operator-gated DEPLOY** — applies additive migration 0024; (2) **live OCR UAT owed** — the WASM paths can't be unit-tested, so UAT must confirm a real PNG/scanned-PDF OCRs **offline (no network)** and a deliberately-bad scan reads `low confidence` (not fed to a draft). **Carryforwards:** pre-fix images stay `not_supported` → fold into MATERIALS-BACKFILL-1 (not a second backfill); mixed text+scan PDFs OCR only when fully empty-text; 'fast' model upgrade path to standard/best if UAT accuracy disappoints; non-blocking follow-up — make `assembleContext` (`context/pipeline.ts`) status-aware for defense-in-depth (the data-layer null already closes the leak). **MATERIALS-DROPZONE-1 (both increments) now COMPLETE.** This state record was written from a separate `lex-next/materials-dropzone-b-state` docs branch to avoid clashing with the parallel session's `state.json` edits.
