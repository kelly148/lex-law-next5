...
## 2026-06-09 (DOC-CLIENT-TARGET-1 FEATURE-COMPLETE + MERGED to `main` — multi-client document→party targeting, the Brown joint-EP unblock; NOT deployed; awaiting operator deploy + Pattern-16 live-verify)

**Disposition.** Operator-directed build of the triad-LOCKED DOC-CLIENT-TARGET-1 disposition across **5 increments / 7 commits**, then operator `push everything + I'll deploy`. **Squash-merged PR #237 → `main` `8964ebf`** (CI green: Lint + Type Check + Tests); branch `lex-next/multi-client-matter` deleted. **NOT flag-gated** (behavior-on-deploy); **NOT deployed** (prod unchanged). The second MULTI-CLIENT-MATTER feature, **CONFLICT-GATE-OVERRIDE-1, is NOT yet built** (its shared conflicts-on-party-add hook shipped here in Inc 1).

**What shipped.** Inc 1 — `document_party` join table + **versioned in-code doc-type config** (keys reconciled to the app's real `documentType` vocabulary: `durable_poa`/`pour_over_will`/`last_will_testament`/`advance_medical_directive`/`revocable_living_trust`/`certificate_of_trust`/`deed`; single-accessor seam for the ADD-DOC-TYPE-1 follow-on) + write-time role validation + the **shared conflicts-on-party-add hook** (wired at `matterIntake.addParty`) + `matter_parties.deletedAt` soft-delete + block-delete guard + `document_party` purge wiring (migration **0023** additive: table + `documents.sourceDocumentId` + `matter_parties.deletedAt`). Inc 2 — `individual_subject` flow: mandatory principal pick (no default, multi-client) / auto-bind sole client; **identity-layer generation scoping** (draft for the bound subject by name — operator decision A) **plus** the deterministic **§6 pre-finalize target-consistency backstop**; the selector + sticky `DraftingTargetHeader` + pair affordance + duplicate guard. Inc 3 — `party_set` joint-trust flow (whole-client-set **creation-time snapshot** binding; "Applies to" label; joint generation). Inc 4 — `RecommendedInstances` per-client enumeration + **target-carrying export filename** + per-instance status (reuses `workflowState`). Inc 5 — `validateTargetingForFinalize` §6 structural hard-blocks (`TARGETING_INVALID`) + `document.migrateTargeting` **operator-gated** retroactive backfill (dry-run→apply; single-client safe-backfill, multi-client FLAGGED never auto-assigned, party_set sets; reuses the pure create resolvers).

**Build state.** main = `8964ebf`; prod unchanged (NOT deployed). ~95 new tests (units + jsdom render + source-audits); tsc + eslint clean; ownerScope ratchet + purge guard green; CI green. The 11 local-only failures are the documented pre-existing Windows-CRLF source-scans (green on Linux CI; proven by baseline stash). **Open items:** (1) **operator-gated DEPLOY** of `main 8964ebf` — a **large accumulated batch since prod ef903be** (reviewer-reliability stack + REVIEW-UX-REDESIGN-1 + DOC-CLIENT-TARGET-1); migration 0023 (and any earlier un-applied additive migrations) auto-apply idempotently via the wired pre-deploy runner; **MODE B** (smoke secrets absent → manual verify/rollback). (2) **Pattern-16 live-verify** post-deploy (create a POA in a 2-client matter → mandatory principal pick → scoped generation → target-carrying export → §6 finalize backstop). (3) Post-deploy **run `document.migrateTargeting`** (dry-run → apply) to backfill pre-existing single-client docs; pre-existing **multi-client individual docs are flagged and need a principal picked before finalize**. (4) **CONFLICT-GATE-OVERRIDE-1** not yet built. **Deferred fast-follows** (noted in commits): designation-binding UI + structural cross-wire guard; deed/`role_sided` + `derived`-inheritance UIs; finalize config-version provenance snapshot + audit-events target/role ledger; `[Change principal]`/`[Change parties]` rebind. **Hazard (recovered):** a parallel UAT session checked the shared working tree out to `main` mid-build; the Inc-1 commit was safe on the branch ref and recovered by re-checkout (→ memory `shared-worktree-hazard`).
