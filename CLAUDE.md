...
- PR open
- Merge to main — only when the Rule-15 auto-merge conditions don't all hold; a reversible-lane PR with green CI, no surfaced decision/fork, and no residual risk auto-merges (Rule 15)
- Prod deploy (`operator approve deploy:`) — deploy is never automated (Railway auto-deploy-on-merge is OFF; merge ≠ deploy)
...
- **auto-merge a reversible-lane PR once CI is green** (Rule 15) — only when no operator decision/fork was surfaced and no residual risk was flagged; otherwise stop for `operator approve accept:`;
...
- squash-merge to main **when the Rule-15 auto-merge conditions don't all hold** (a clean reversible-lane PR with green CI, no surfaced decision/fork, and no residual risk auto-merges per Rule 15; everything else still stops here);
